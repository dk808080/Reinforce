...
function UnsolvedQueries() {
    ...
    useEffect(() => {
        setAuthToken();
        axios
            .get(`${getServerUrl()}/api/query/unsolved-queries`)
            .then((res) => {
            ...
            <div id="accordion-2" class="accordion accordion-spaced">
                ...
                <div class="card">
                    <div class="card-header py-4" id={`heading-2-${i}`} data-toggle="collapse" role="button" data-target={`#collapse-2-${i}`} aria-expanded="false" aria-controls={`collapse-2-${i}`}>
                        <div className="mb-3">
                            Posted on : <span class="badge badge-soft-primary badge-pill">{new Date(query.datePosted).toLocaleString()}</span>
                        </div>
                ...
                <div class="card bg-primary shadow-lg rounded-lg border-0">
                    <div class="px-4 py-5 text-center">
                        <div class="h3 text-white">
                            No unsolved queries are there!!
                        </div>
                        <Link to="/submit-query"><button type="button" class="btn btn-dark" >
                            Submit a query
                        </button></Link>
                    </div>
                </div>
